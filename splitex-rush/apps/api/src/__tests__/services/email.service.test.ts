...
describe('EmailService', () => {
  ...
  describe('sendInvitationEmail', () => {
    ...
    it('should use custom APP_URL for accept link', async () => {
      process.env.APP_URL = 'https://splitex.app';
      sendMailMock.mockResolvedValue({ message: '{}' });
      ...
      const callArgs = sendMailMock.mock.calls[0][0];
      expect(callArgs.html).toContain('https://splitex.app/invitations/accept?token=abc123token');
    });

    it('should use custom SMTP_FROM address', async () => {
      process.env.SMTP_HOST = 'smtp.example.com';
      process.env.SMTP_FROM = 'team@splitex.app';
      sendMailMock.mockResolvedValue({ messageId: '<id>' });
      ...
      const callArgs = sendMailMock.mock.calls[0][0];
      expect(callArgs.from).toContain('team@splitex.app');
    });
