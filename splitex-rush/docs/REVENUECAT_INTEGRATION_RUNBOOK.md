# RevenueCat Integration Runbook (Splitex)
...
## Prerequisites
1. Splitex API running locally (`rush dev:api`).
...
- RevenueCat `app_user_id` must match Splitex `userId` persisted in API user document.
