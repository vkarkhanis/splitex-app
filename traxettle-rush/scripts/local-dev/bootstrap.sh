...

{
  ...
  echo "BOOTSTRAP_FIREBASE_API_KEY=\"$EXTRACTED_API_KEY\""
  echo "BOOTSTRAP_FIREBASE_APP_ID=\"$EXTRACTED_APP_ID\""
  echo "BOOTSTRAP_FIREBASE_MESSAGING_SENDER_ID=\"$EXTRACTED_MESSAGING_SENDER_ID\""
  echo "BOOTSTRAP_FIREBASE_PROJECT_ID=\"$EXTRACTED_PROJECT_ID\""
  echo "BOOTSTRAP_FIREBASE_STORAGE_BUCKET=\"$EXTRACTED_STORAGE_BUCKET\""
  echo "BOOTSTRAP_FIREBASE_AUTH_DOMAIN=\"$EXTRACTED_AUTH_DOMAIN\""
} > "$BOOTSTRAP_ENV"
...
if [ -n "$EXTRACTED_API_KEY" ]; then
  ok "Firebase API key extracted"
fi
if [ -n "$EXTRACTED_APP_ID" ]; then
  ok "Firebase app ID extracted"
fi
