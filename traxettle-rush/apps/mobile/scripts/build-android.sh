...
# Load RevenueCat keys from gitignored rc_<env>.properties.
...
if [ "$PROFILE" = "debug" ]; then
  if [ -z "${EXPO_PUBLIC_REVENUECAT_GOOGLE_KEY:-}" ]; then
    warn "EXPO_PUBLIC_REVENUECAT_GOOGLE_KEY missing from rc_${RC_ENV}.properties — Pro purchases won't work in this build."
  fi
else
  rc_require_nonempty EXPO_PUBLIC_REVENUECAT_GOOGLE_KEY
fi

ok "EXPO_PUBLIC_APP_ENV=$EXPO_PUBLIC_APP_ENV"
[ "$PROFILE" != "debug" ] && ok "EXPO_PUBLIC_API_URL=$EXPO_PUBLIC_API_URL"
ok "RevenueCat config loaded from rc_${RC_ENV}.properties"
...
# Write TS fallback config so release APK always carries RC keys even if Expo
# dotenv inlining is skipped in some native build paths.
cat > "$BUILD_ENV_TS" <<EOF
/**
 * Auto-generated by build-android.sh for profile: $PROFILE
 * DO NOT COMMIT.
 */
