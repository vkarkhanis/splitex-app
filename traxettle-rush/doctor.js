 ...
 * 
 * Usage: node doctor.js [environment]
...
function detectEnvironment() {
  const env = process.argv[2] || process.env.NODE_ENV || 'local';
...
async function askYesNo(question) {
  while (true) {
    ...
    colorLog('red', 'Please answer y/yes or n/no');
  }
}
...
class TraxettleDoctor {
  ...
  async checkBackendDependencies() {
    ...
    try {
      ...
      const installed = await askYesNo('Have you installed backend dependencies with rush install?');
      if (!installed) {
    ...
    if (hasEnvFile) {
      ...
      const setup = await askYesNo('Have you set up backend environment variables?');
      if (!setup) {
  ...
  async checkWebDependencies() {
    ...
    try {
      ...
      const installed = await askYesNo('Have you installed web dependencies with rush install?');
      if (!installed) {
        colorLogLink('Install dependencies', 'Run: rush install');
  ...
  async checkMobileDependencies() {
    ...
    if (commandExists('npx') && commandExists('expo')) {
      ...
      const setup = await askYesNo('Is Expo CLI available via npx?');
      if (!setup) {
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      ...
      if (androidHome && fileExists(androidHome)) {
        ...
        const setup = await askYesNo('Have you set up Android SDK for mobile builds?');
        if (!setup) {
    ...
    if (process.platform === 'darwin') {
      if (commandExists('xcodebuild')) {
        ...
        const setup = await askYesNo('Have you installed Xcode for iOS builds?');
        if (!setup) {
  ...
  async checkFirebaseSetup() {
    ...
    if (fileExists(serviceAccountPath)) {
      ...
      const setup = await askYesNo(`Have you created Firebase project "${projectId}" and downloaded service account key?`);
      if (!setup) {
  ...
  async checkRevenueCatSetup() {
    ...
    if (revenueCatConfigured) {
      ...
      const setup = await askYesNo('Have you set up RevenueCat for in-app purchases?');
      if (!setup) {
  ...
  async checkEmailSetup() {
    ...
    if (emailConfigured) {
      ...
      const setup = await askYesNo('Have you set up SMTP for email notifications?');
      if (!setup) {
    ...
    if (smtpHost && smtpHost.includes('gmail')) {
      ...
      const hasAppPassword = await askYesNo('Have you generated a Gmail app-specific password?');
      if (!hasAppPassword) {
  ...
  async checkBuildConfiguration() {
    ...
    if (fileExists(keystorePath)) {
      ...
      const setup = await askYesNo('Have you generated Android keystore for release builds?');
      if (!setup) {
...
async function main() {
  try {
    ...
    const doctor = new TraxettleDoctor(environment);
    await doctor.run();
  } catch (error) {
