...
set_entitlement() {
  ...
  local resp
  resp="$(curl -sS -o /tmp/traxettle_entitlement_set.json -w "%{http_code}" \
  ...
  if [[ "$resp" != "200" && "$resp" != "201" ]]; then
    ...
    echo "Body:"
    cat /tmp/traxettle_entitlement_set.json || true
...
health_code="$(curl -sS -o /tmp/traxettle_health.json -w "%{http_code}" "${API_BASE}/health")"
...
free_code="$(create_event_with_fx /tmp/traxettle_fx_free.json)"
...
if [[ "$pro_code" == "201" || "$pro_code" == "200" ]]; then
  ...
  echo "Body:"
  cat /tmp/traxettle_fx_pro.json || true
...
echo "Artifacts:"
echo "  /tmp/traxettle_entitlement_set.json"
echo "  /tmp/traxettle_fx_free.json"
echo "  /tmp/traxettle_fx_pro.json"
