...
This document is mapped to your current code in `splitex-rush` and is written for minimal structural change.

## 1) Files to modify

## A. Shared types
1. `splitex-rush/libraries/shared/src/index.ts`
...
## B. API dependencies
1. `splitex-rush/apps/api/package.json`
- Add:
  - `razorpay`
  - `stripe`

## C. API service layer (new files)
1. `splitex-rush/apps/api/src/services/payment-gateway/types.ts`
- Add interfaces:
...
2. `splitex-rush/apps/api/src/services/payment-gateway/razorpay.gateway.ts`
...
3. `splitex-rush/apps/api/src/services/payment-gateway/stripe.gateway.ts`
...
4. `splitex-rush/apps/api/src/services/payment-gateway/payment-gateway.factory.ts`
- Provider selection:
  - `INR => RazorpayGateway`
  - otherwise `StripeGateway`

## D. API existing services
1. `splitex-rush/apps/api/src/services/fx-rate.service.ts`
...
2. `splitex-rush/apps/api/src/services/settlement.service.ts`
...
## E. API routes
1. `splitex-rush/apps/api/src/routes/settlements.ts`
...
2. New file: `splitex-rush/apps/api/src/routes/payment-webhooks.ts`
- Add endpoints:
  - `POST /razorpay`
  - `POST /stripe`
- Route flow:
...
3. `splitex-rush/apps/api/src/index.ts`
- Register:
...
## F. Web app
1. `splitex-rush/apps/web/src/app/events/[eventId]/page.tsx`
...
## G. Mobile app
1. `splitex-rush/apps/mobile/src/screens/EventDetailScreen.tsx`
- Existing `handlePay` currently only calls API and optimistic-updates status.
- Replace with:
  1. call `/api/settlements/:id/pay`
  2. read `checkoutUrl`
...
## H. Tests
1. `splitex-rush/apps/api/src/__tests__/services/settlement.service.test.ts`
...
2. New file:
- `splitex-rush/apps/api/src/__tests__/routes/payment-webhooks.test.ts`
- Cases:
  - valid signature -> completed
  - duplicate webhook -> no duplicate state transition
  - invalid signature -> 401

3. `splitex-rush/apps/web` and `splitex-rush/apps/mobile`
...
node common/scripts/install-run-rush.js build -t @splitex/shared
node common/scripts/install-run-rush.js build -t @splitex/api
node common/scripts/install-run-rush.js test -t @splitex/api
```

Optional local app checks:

```bash
cd /Users/vkarkhanis/workspace/Splitex/splitex-rush && rush dev:web
cd /Users/vkarkhanis/workspace/Splitex/splitex-rush && rush dev:mobile
```
