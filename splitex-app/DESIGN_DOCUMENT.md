# Splitex - Expense Splitting Application Design Document

## 1. System Overview

Splitex is a comprehensive expense splitting application that supports both web (React/Next.js) and mobile (React Native) platforms. The application enables users to create events/trips, invite participants, track expenses, and settle payments efficiently using a greedy algorithm for optimal settlement.
...
- **UI Components**: styled-components + @splitex/ui shared library (Web & Mobile)
- **Icons**: Lucide React
...
- **Monorepo Structure**:
  - `apps/web` — Next.js web application (@splitex/web)
  - `apps/api` — Express API backend (@splitex/api)
  - `apps/mobile` — React Native/Expo mobile app (@splitex/mobile)
  - `libraries/shared` — Shared types and utilities (@splitex/shared)
  - `libraries/ui` — Shared UI components, theme, and toast system (@splitex/ui)
...
- **Week 1-2**: Project setup with Rush monorepo, shared UI library (@splitex/ui) with styled-components theme system
